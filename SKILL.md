...
- 无系统 JDK 时在线下载 **Eclipse Temurin JDK 25**（GPLv2 + Classpath Exception 协议，无授权问题）：官方源失败自动切清华镜像，缓存到 `~/.erupt-skill/runtime` 后续复用
- Maven 3.9 自动下载缓存，项目依赖默认走阿里云镜像
- 本地 `vendor/` 目录（不入 git）可选放置离线包：`vendor/jdk/jdk25-<os>-<arch>.tar.gz` 与 `vendor/m2/m2-repo.tar.gz.part-*`（可用 `scripts/build-m2-seed.sh` 生成），存在时优先使用、完全免下载
...
| Windows 环境 | 脚本需在 Git Bash / MSYS 中运行（Claude Code on Windows 自带 Git Bash）；脚本跑不通时可自装 [JDK 17](https://adoptium.net) 与 [Maven](https://maven.apache.org) 后直接 `mvn spring-boot:run` |
