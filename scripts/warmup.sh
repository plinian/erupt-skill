...
# Key the marker on template pom content: template upgrades (e.g. erupt version
# bump) trigger a fresh warmup automatically
pom_sum="$(cksum "$TEMPLATE_DIR/pom.xml" | awk '{print $1}')"
marker="$SKILL_HOME/.warmed-$pom_sum"
if [ -f "$marker" ]; then
    log "Dependencies already warmed, skipping"
    exit 0
fi
...
log "Warming up Maven dependencies (runs in background, does not block requirement analysis or code generation)..."
cd "$work"
